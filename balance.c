...
void LearnVocabFromTrainFile(struct train_params *params) {
  ...
  if (unk_id<0){
    ...
    fprintf(stderr, "! Can't find %s in the vocab file %s, [hash id found] adding ...\n", unk_word, params->train_file);
    a = AddWordToVocab(unk_word, params);
    unk_id = params->vocab_hash[GetWordHash(unk_word)];
    fprintf(stderr, "  unk_id = %d\n", unk_id);
    params->vocab[a].cn = min_count;
...
void *TrainModelThread(void *id) {
  ...
  int src_read = 1, tgt_read = 1, parallel_read = 1;
  long long line_num = line_blocks[(long long) id];
  long long next_block_line_num = line_blocks[(long long) id+1];
  // printf("\nThread%lld:%lld, %lld", id, next_block_line_num);
  ...
  while (1) {
   ...
   // printf("\n(word count) Thread%lld:%lld", (long long) id, src_word_count);
   /*
   if (feof(src_fi)) {
   printf("\n(Eof) Thread%lld:%lld", (long long) id, sent_id);
   break;
   }
   if (src_word_count > src->train_words / num_threads) {
   printf("\n(limit) Thread%lld:%lld", (long long) id, sent_id);
   break;
   }*/

   if(line_num >= src->num_lines){
     src_read = 0;
   }
   if(line_num >= tgt -> num_lines){
    tgt_read = 0;
   }
   if(line_num >= align_num_lines){
    parallel_read = 0;
   }
   if (src_read == 0 && tgt_read == 0 && parallel_read == 0){
    printf("\n(all 0) Thread%lld:%lld", (long long) id, sent_id);
    break;
   }
   if(line_num >= next_block_line_num){
    printf("\n(next block) Thread%lld:%lld", (long long) id, sent_id);
    break;
   }
      ...
      if (align_opt) { // use unsupervised alignments
        ...
        for (src_pos = 0; src_pos < src_sentence_orig_length; ++src_pos) {
          ...
          if (count>0 && tgt_id_map[tgt_pos]>=0){
           // printf("\n[%lld]....process (pair) sentence id %lld\n", (long long) id, sent_id);
  ...
  if (parallel_read == 0 && is_bi && align_opt){
    fseek(src_fi, 0, SEEK_SET);
    fseek(tgt_fi, 0, SEEK_SET);
    fseek(align_fi, 0, SEEK_SET);
    line_num = 0;
    src_read = 1;
    tgt_read = 1;
    parallel_read = 1;
    ...
    /* CHECK IF REQUIRED FOR ALPHA UPDATE
        #ifdef DEBUG
...
void MonoInit(struct train_params *params, long long train_words){
  ...
  // todo ComputeBlockStartPoints(params->train_file, num_threads, &params->line_blocks, &params->num_lines);
...
//todo
void mapOffsets(long long *offsets, FILE *fi, int num_blocks, char f_name){

    fseek(fi, 0 , SEEK_SET);
    // printf("\n\nFilename:%c", f_name);
    long i = 0;
    long long bytes = 0, line_counter = 0;
    char l[MAX_SENT_LEN];

    while (i<num_blocks && fgets(l, MAX_SENT_LEN, fi) != NULL){
        if (line_blocks[i] == line_counter){
            offsets[i] = bytes;
            // printf("\nAlign offset: %lld for", offsets[i]);
            // printf("Line number: %lld", line_blocks[i]);
            i += 1;
        }
        bytes += strlen(l);
        line_counter += 1;
        /*if (f_name =='a'){
         //printf("\n(line)%s", l);
         //printf("\n%lld (line num), %lld (bytes)", line_counter, bytes);
        }*/
    }
}

//todo
void ComputeLineOffsets(int num_blocks){

    int i = 0;
    FILE *src_fi = fopen(src->train_file, "rb");
    FILE *tgt_fi = fopen(tgt->train_file, "rb");
    FILE *align_fi = fopen(align_file, "rb");

    src->line_blocks = malloc(num_blocks * sizeof(long long));
    tgt->line_blocks = malloc(num_blocks * sizeof(long long));
    align_line_blocks = malloc(num_blocks * sizeof(long long));

    mapOffsets(src->line_blocks, src_fi, num_blocks, 's');
    mapOffsets(tgt->line_blocks, tgt_fi, num_blocks, 't');
    mapOffsets(align_line_blocks, align_fi, num_blocks, 'a');

    for (i=0; i<=num_blocks; i++){

        printf("\n\n\nSrc offset: %lld for", src->line_blocks[i]);
        printf("\n\n\nTgt offset: %lld for", tgt->line_blocks[i]);
        printf("\n\n\nAlign offset: %lld for", align_line_blocks[i]);
        //printf("Line number: %lld", line_blocks[i]);
    }
}

//todo
int getLines(char* file_name){
    FILE *file;
...
//todo
void ComputeBlockStartPoints(int num_blocks) {

  long i;

  src->num_lines = getLines(src->train_file);
  tgt->num_lines = getLines(tgt->train_file);
  align_num_lines = getLines(align_file);

  long long max_lines = tgt->num_lines;
  if (src->num_lines > tgt->num_lines){
    max_lines = src->num_lines;
  }
  printf("# ComputeBlockStartPoints: num_blocks=%d\n", num_blocks);
  line_blocks = malloc((num_blocks+1)* sizeof(long long));

  // printf("\n\n\nDEBUGGING");

  long long block_size = max_lines/num_blocks + 1;

  // printf("\n\n\n%lld BLOCK SIZE", block_size);

  line_blocks[0] = 0; line_blocks[num_blocks] = max_lines;
  for (i=1; i < num_blocks; i++){
    line_blocks[i] = line_blocks[i-1] + block_size;
  }
}
...
void TrainModel() {
  ...
  if(is_bi){
    ...
    //todo assert(src->num_lines==tgt->num_lines);
  }
  if (align_opt) {
    //todo ComputeBlockStartPoints(align_file, num_threads, &align_line_blocks, &align_num_lines);
    //todo assert(src->num_lines==align_num_lines);
    // num_parallel_lines = getLines(align_file); //todo
  }
  ComputeBlockStartPoints(num_threads); //todo
  ComputeLineOffsets(num_threads); //todo
