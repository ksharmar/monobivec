...
args="-src-train $trainPrefix.$srcLang -tgt-train $trainPrefix.$tgtLang -src-lang $srcLang -tgt-lang $tgtLang -output $outputDir/out -cbow $isCbow -size $dim -window 5 $negStr -threads $numThreads -binary 0 -iter $numIter -eval 0 $alphaStr $sampleStr $monoStr $otherOptStr"
...
if [ $alignOpt -ge 1 ]; then
  execute_check "" "time $SCRIPT_DIR/balance -align $trainPrefix.aligned -align-opt $alignOpt $args"
else
  execute_check "" "time $SCRIPT_DIR/balance $args"
fi
